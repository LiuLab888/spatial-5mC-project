...
cat > ${name}_${part}_split3.sh << EOF

#!/bin/sh
#PBS -q ${queue}
#PBS -l nodes=1:ppn=${ppn},mem=${mem}gb,walltime=200:00:00
#PBS -e ${Output_path}/split3_${part}_${name}.err
#PBS -o ${Output_path}/split3_${part}_${name}.out
#HSCHED -s hschedd
#PPN limit ${ppn}
...
sh /p300s/liujiang_group/tangym/Analysis_data/spatial_methylation_bismark_integration_strand/spatial_split_core_3.sh ${name} \${i} ${work_path} 
