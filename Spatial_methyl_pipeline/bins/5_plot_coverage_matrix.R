...
pdf("coverage.pdf",width = round(nchannel_B*12/96)+1,height = round(nchannel_A*12/96))
...
library(circlize)
svg("coverage_level.svg",width = round(nchannel_B*12/96)+1,height = round(nchannel_A*12/96))
Heatmap(all_combi.m,cluster_rows = F,cluster_columns = F,
        rect_gp = gpar(col = "grey", lwd = 0.5),
        show_row_names = F, show_column_names = F,
        heatmap_legend_param = list(legend_height = unit(3, "cm"),
